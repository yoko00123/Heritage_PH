...
namespace InSys.Helpers
{
    ...
    ///
    [InvariantCulture]
    public class InfoSet : IDisposable
    {
        ...
        public InfoSet(int ID_Menu, IQueryArgs sql, HttpContext httpcontext)
        {
            //log4net.ThreadContext.Properties["Class"] = "InfoSet";
            ...
            this.ds = new DataSet("InfoSet");
            this.rowDeleted = new Dictionary<string, List<int?>>();

            Status($"ID_Menu : { ID_Menu }");
        }
        ...
        protected void PrepareTableHeader()
        {
        ...
        protected void PrepareTableDetails(string TableName)
        {
            foreach (tMenuDetailTab tab in menu.tMenuDetailTab.Where(x => x.TableName == TableName))
                ds.Tables.Add(new ZDataTable(conn, tab.TableName, tab.HasAuditTrail.ToBool(), HostName, Session["ID_Session"].ToInt32(), menu.tMenu.Name));
        }
        ...
        protected void SetRelations(tMenuDetailTab x)
        {
            var master = ds.Tables[menu.tMenu.TableName];
            foreach (DataRow dr in ds.Tables[x.TableName].Rows)
                //dr[x.ChildColumn] = master.Rows[0][x.ParentColumn]; Mod Weng
        ...
        public void SaveHeader(string data)
        {
            SqlTransaction tran = default(SqlTransaction);
            try
            {
        ...
        public void SaveDetail(string TableName, string Data, string RowDeleted, string RowHeader)
        {
            SqlTransaction tran = default(SqlTransaction);
            try
            {
                ...
                Status("Preparing Tables");
                this.PrepareTableHeader();
                this.PrepareTableDetails(TableName);

                Status("Deserializing Data");
                var json = RowHeader.ToObject<Pair>(); //(JObject)JsonConvert.DeserializeObject(RowHeader);

                var dt = ds.Tables[menu.tMenu.TableName] as ZDataTable;
                AddRow(dt, json);


                dt = ds.Tables[TableName] as ZDataTable;
                if (dt == null) TryCreateTable(TableName);
                var jsons = Data.ToObject<PairCollection>();
                foreach (var f in jsons)
                    AddRow(dt, f);

                Status("Deserializing Row Detail");
                rowDeleted.Add(TableName, RowDeleted.ToObject<List<int?>>());

                Status("Updating Relations");
                this.SetRelations(menu.tMenuDetailTab.Where(x => x.TableName == TableName).Single());
        ...
        protected void AddRow(ZDataTable dt, Pair f)
        {
            var Column = default(string);
            try
            {
                ...
                foreach (DataColumn dc in dt.Columns)
                {
                    try
                    {
                        ...
                        f.TryGetValue(dc.ColumnName, out obj);
        ...
        public int? ID
        {
            get
            {
                return this.row["ID"].ToInt32();  //.Field<int>("ID");
            }
        }
